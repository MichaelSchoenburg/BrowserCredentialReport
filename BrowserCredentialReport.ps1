...
if (Test-Path -Path $ToolPath) {
    ...
    Log "Writing files for Nir Soft tools..."
    ...
    foreach ($u in $UserFolders) {
        ...
        if (Test-Path -Path $ChromePath) {
            Log "Processing Chrome for $( $u.Name ): $ChromePath"
            ...
            Log "No Chrome profile found for $( $u.Name )."
        }

        # Internet Explorer & MS Edge
        if (Test-Path -Path $EdgePath) {
            Log "Processing Edge for $( $u.Name ): $EdgePath"
